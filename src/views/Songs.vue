    ...
    <div id="songs-view" @scroll="handleScroll">
        <div class="wrapper-header">
            ...
            <div class="wrapper-search">
                <input id="input-search" placeholder="Search by title..."
                    v-model="searchInput" v-on:change="filterBySearch()" />
            </div>
            <div class="wrapper-settings">
                <button id="btn-show-favorites" v-bind:class="{ active: show_favorites }" 
                    @click="changeFavoritesState()">Show Favorites</button>
            </div>
        </div>
        <div class="wrapper-songs">
            <table cellspacing="0" cellpadding="0">
                <tr ref="header">
                    <th id="th-id" >#</th>
                    <th id="th-title" @click="sortByName()" 
                        v-bind:class="{ active: isSortedByName }">
                        Title
                        <IconCaretUp  v-if="isSortedByName" style="stroke: red" 
                            v-bind:class="{'flip-vertical':isReversed}"/>
                    </th>
                    <th id="th-artist">Artist</th>
                    <th id="th-album">Album</th>
                    <th id="th-duration" @click="sortByDuration()" v-bind:class="{ active: isSortedByDur }">
                        Duration
                        <IconCaretUp v-if="isSortedByDur" style="stroke: red" 
                            v-bind:class="{'flip-vertical':isReversed}"/>
                    </th>
                </tr>
                
                <tr class="song" v-for="(value, index) in player.playlist" v-on:dblclick="selectSong(value)" 
                    v-bind:class="{ active: player.getNowPlayingSongId()==value.id?true:false }"> 
                    <td id="td-index">
                    ...
                    <td id="td-duration">
                        ...
                        <IconHeart @click="addSongToFavorites(value)" 
                            v-bind:class="{active: auth.user.favorite_songs.some(el => el.id === value.id)}"/>
                    </td>
                </tr>
            </table>
        </div>
